...
function Dashboard({ data, user, nav }) {
  ...
  const readyToProcess = user.role === "finance" ? mine.filter(d => ["doc_complete","approved"].includes(d.status) && !d.settled) : [];
    ...
    <div>
      ...
      {readyToProcess.length > 0 && (
        <div className="card mb4">
          <div className="ch" style={{background:"linear-gradient(135deg,#1e1b4b,#312e81)",borderRadius:"var(--r) var(--r) 0 0"}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:15}}>⚡</span>
              <h3 style={{color:"#fff",fontWeight:800}}>Perlu Diproses Sekarang</h3>
              <span style={{background:"#ef4444",color:"#fff",fontSize:10,fontWeight:800,padding:"2px 8px",borderRadius:10}}>{readyToProcess.length}</span>
            </div>
            <span style={{fontSize:11.5,color:"rgba(255,255,255,.5)"}}>Dokumen sudah lengkap dari GA</span>
          </div>
          <div style={{padding:"4px 0"}}>
            {readyToProcess.map(d => (
              <div key={d.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 18px",borderBottom:"1px solid var(--ln)"}}>
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div className="bold trunc" style={{fontSize:13,maxWidth:300}}>{d.submitter} — {d.purpose}</div>
                  <div style={{fontSize:11,color:"var(--i3)",marginTop:2}}>{d.destination} · {fd(d.dateStart)} – {fd(d.dateEnd)} · <strong style={{color:"var(--ink)"}}>{rp(d.amount)}</strong></div>
                </div>
                <button className="btn bp sm" onClick={() => nav("dashboard", d.id)} style={{flexShrink:0}}>
                  Proses →
                </button>
              </div>
            ))}
          </div>
          {readyToProcess.length > 3 && (
            <div style={{padding:"10px 18px",textAlign:"right"}}>
              <button className="btn bo sm" onClick={() => nav("monitor")}>Lihat Semua di Monitor →</button>
            </div>
          )}
        </div>
      )}
