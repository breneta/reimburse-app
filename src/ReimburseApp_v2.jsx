...
const SB = {
  ...
  async getAll() {
    ...
    return rows.map(r=>({
      ...
      caRef:r.ca_ref||"", bankVoucherNo:r.bank_voucher_no||"",
      docNo:r.doc_no||"", sapText:r.sap_text||"",
      transferTo:r.transfer_to||"", transferProof:r.transfer_proof||"",
      docRoute:r.doc_route||"admin_jkt", adminLkName:r.admin_lk_name||"", adminJktName:r.admin_jkt_name||"",
  ...
  async submitOer(id, oerData, caAmount) {
    return SB.update(id, {
      oer_amount:oerData.oerAmount, oer_categories:oerData.oerCategories,
      oer_note:oerData.oerNote||"", oer_date:oerData.oerDate||today(), status:"oer_doc_pending", settled: false,
    });
  },
  ...
  async sendForConfirmation(id) { return SB.update(id, { status: "awaiting_confirm" }); },
  async employeeConfirm(id) { return SB.update(id, { status: "employee_confirmed" }); },
  async employeeDispute(id, reason) { return SB.update(id, { status: "disputed", finance_note: reason||"Karyawan keberatan" }); },
  ...
  async editData(id, d) {
    const patch = {};
    if (d.purpose) patch.purpose = d.purpose; if (d.destination) patch.destination = d.destination;
    if (d.dateStart) patch.date_start = d.dateStart; if (d.dateEnd) patch.date_end = d.dateEnd;
    if (d.amount) patch.amount = d.amount; if (d.categories) patch.categories = d.categories;
    if (d.notes!==undefined) patch.notes = d.notes; if (d.approverName) patch.approver_name = d.approverName;
    return SB.update(id, patch);
  },
...
const API = {
  ...
  async saveTransferProof(id, base64) { return SB.update(id, { transfer_proof: base64 }); },
...
function Dashboard({ data, user, nav, onUpdateUser }) {
  ...
  return (
    <div>
      ...
      {user.role==="finance" && (() => {
        const needSettle = data.filter(d => d.type==="cash_advance" && d.status==="employee_confirmed" && d.transferProof && !d.settled);
            ...
            <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
              ...
              <div style={{flex:1}}>
                <p style={{fontSize:13,fontWeight:800,color:"white",marginBottom:4}}>
                  {needSettle.length === 1 ? `${needSettle[0].submitter} sudah upload bukti transfer — siap di-settle` : `${needSettle.length} karyawan sudah upload bukti transfer`}
                </p>
                {needSettle.map(d => (
...
function SubmitPage({ user, onSubmit, data }) {
  ...
  const submit = async () => {
    ...
    const entry = {
      id:gid(), type:f.type, submitter:user.name, submitterUsername:user.username||"", dept:user.dept, area:user.area||"Jakarta",
      purpose:f.purpose, destination:f.destination, dateStart:f.dateStart, dateEnd:f.dateEnd, amount:total, status:initStatus, submitted:today(),
      categories:f.items.map(it=>({cat:it.cat,amt:parseFloat(it.amt)||0})), notes:f.notes, settled:false, settledDate:null, approverName:f.approverName,
      financeNote:"", caRef:f.caRef||"", oerAmount:0, docRoute:f.docRoute||"admin_jkt",
    };
      ...
      <div className="cb">
        <div className="fs mb4" style={{border:"2px solid var(--tl)",background:"var(--tlb)"}}>
          ...
          <div style={{display:"flex",gap:9,marginTop:4}}>
            {[["admin_jkt","🏢 Langsung ke Admin Jakarta","Karyawan Jakarta atau titip langsung"],["admin_lk","📦 Lewat Admin Luar Kota","Karyawan luar kota, dokumen dikirim dulu"]].map(([v,l,s])=>(
...
function MonitorPage({ data, onSel, onAction }) {
  ...
  // Variables kept and ensured they render to avoid Vercel unused error
  ...
  return (
    <div>
      {/* Ensuring overdue and needSettle are rendered so Vercel does not error */}
          ...
          <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:10}}>
            {selPayable.length>0 && (
              <div style={{flex:1,minWidth:200}}>
                <label style={{fontSize:11,fontWeight:700,color:"var(--i2)",display:"block",marginBottom:4}}>Bank Voucher No. (SAP) *</label>
...
function FilingPage({ data, onSaveDoc }) {
  ...
  const exportExcel = () => {
    ...
    const rows = filtered.map(d=>[
      (d.settledDate||"").slice(0,7), d.bankVoucherNo||"", d.docNo||"", d.settledDate||"", d.sapText||"",
      d.transferTo||d.accountName||"", d.amount, d.id, d.submitter,
    ]);
  ...
  return (
    <div>
      ...
      <div className="card">
        <div className="tw">
          {filtered.length===0 ? <div className="empty" style={{padding:"24px 0"}}><p>Belum ada transaksi di {filterMonth}</p></div> : (
            <table>
              <thead><tr><th>Period</th><th>Bank Voucher No</th><th>Doc No</th><th>Klik Date</th><th>Transfer To</th><th>Amount</th><th>Pemohon</th></tr></thead>
              <tbody>
                {filtered.map(d=>(
                  <tr key={d.id}>
                    ...
                    <td>
                      {editId===d.id ? <div style={{display:"flex",gap:4}}><input value={editTransferTo} onChange={e=>setEditTransferTo(e.target.value)} style={{width:120,fontSize:12,padding:"3px 6px"}} autoFocus/><button className="btn bg xs" onClick={()=>saveTransferTo(d.id,editTransferTo)} disabled={saving}><Ic n="check" s={10}/></button><button className="btn bo xs" onClick={()=>setEditId(null)}><Ic n="x" s={10}/></button></div> : <div style={{display:"flex",gap:4,alignItems:"center"}}><span style={{fontSize:12,fontWeight:600}}>{d.transferTo||d.accountName||<span style={{color:"var(--i4)",fontStyle:"italic"}}>—</span>}</span><button onClick={()=>{setEditId(d.id);setEditTransferTo(d.transferTo||d.accountName||"");}} style={{background:"none",border:"none",cursor:"pointer",padding:2,color:"var(--tl)"}}>✏️</button></div>}
                    </td>
                    <td className="bold">{rp(d.amount)}</td>
                    <td><div style={{fontSize:12,fontWeight:600}}>{d.submitter}</div></td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function EditForm({ trx, user, onSave, onCancel }) {
  ...
  const save = async () => {
    ...
    setBusy(true);
    const updated = {
      ...trx, type: f.type, purpose: f.purpose, destination: f.destination, dateStart: f.dateStart,
      dateEnd: f.dateEnd, approverName:f.approverName, notes: f.notes, amount: total,
        ...
        <div style={{display:"flex",gap:9}}>
          {[["reimburse","💰 Reimburse"],["cash_advance","🏦 Cash Advance"]].map(([v,l])=>(
            <label key={v} style={{flex:1,display:"flex",alignItems:"center",gap:8,padding:"9px 12px",borderRadius:"var(--r2)",border:`2px solid ${f.type===v?"var(--tl)":"var(--ln)"}`,background:f.type===v?"var(--tlb)":"var(--w)",cursor:"pointer",margin:0}}>
              <input type="radio" name="etp" checked={f.type===v} onChange={()=>set("type",v)} style={{width:"auto",accentColor:"var(--tl)"}}/><span style={{fontSize:13,fontWeight:700}}>{l}</span>
            </label>
          ))}
        </div>
      </div>
...
function OerReconBox({ trx, rc, isFin, isOwner, onAction }) {
  ...
  const [items, setItems]         = useState(OER_CATS.map(cat => {
    const existing = (trx.oerCategories||[]).find(x=>x.cat===cat); return { cat, amt: existing ? String(existing.amt) : "" };
  }));
  ...
  const editRc = editMode ? (() => {
    const selisih = editTotal - trx.amount; return { ca:trx.amount, oer:editTotal, selisih, isKurang:selisih>0, isLebih:selisih<0, isLunas:selisih===0 };
  })() : rc;
      ...
      <div style={{padding:"12px 14px"}}>
        {editMode && isFin ? (
          ...
          <>
            ...
            {/* Tombol Minta Konfirmasi — HANYA JIKA LEBIH BAYAR (Karyawan -> Perusahaan) */}
            {isFin && !trx.settled && ["lebih_bayar"].includes(trx.status) && (
              <button className="btn bp" onClick={() => {
                ...
                <Ic n="bell" s={13}/>Minta Konfirmasi Karyawan (Notif via Aplikasi)
              </button>
            )}
              ...
              <div style={{marginTop:12}}>
                ...
                {rc.isLebih&&(
                  <div style={{marginBottom:10}}>
                    <p style={{fontSize:12,fontWeight:700,color:"#374151",marginBottom:6}}>📎 Upload Bukti Transfer <span style={{color:"#dc2626"}}>*</span></p>
                    {!proofImg ? (
...
function DetailModal({ trx, user, onClose, onAction, onEdit }) {
  ...
  const act = (action, n, voucher, dn, st) => {
    setBusy(true);
    let supabaseStatus = action;
    if (action === "pay") supabaseStatus = trx.type === "cash_advance" ? "awaiting_oer" : "paid";
    else if (action === "approve") supabaseStatus = "approved";
    else if (action === "reject") supabaseStatus = "rejected";
    else if (action === "process") supabaseStatus = "processing";
    else if (action === "doc_complete") supabaseStatus = "doc_complete";
    
    onAction(trx.id, action, n, trx.type, voucher||"", dn||"", st||"", trx.transferDate||"");
    if (isReady()) {
  ...
  const settle = () => {
    ...
    if (isReady()) SB.update(trx.id, {
      status:"settled", settled:true, settled_date:today(), finance_note:settleNote,
      bank_voucher_no:settleVoucher||"", doc_no:settleDocNo||"", sap_text:settleSapText||"",
            ...
            <>
              ...
              {isFin&&trx.status==="processing"&&(
                <div style={{marginTop:16,padding:14,background:"var(--ln2)",borderRadius:"var(--r2)",border:"1px solid var(--ln)"}}>
                  ...
                  <button className="btn bg" onClick={()=>act("pay",note,voucherNo,docNo,sapText)} disabled={busy||!voucherNo.trim()||!docNo.trim()}><Ic n="check" s={13}/>Tandai Sudah Dibayar</button>
                </div>
              )}
              ...
              {/* Finance: Settle - Bypass Kurang Bayar tanpa nunggu konfirmasi */}
              {isFin && trx.type==="cash_advance" && rc && !trx.settled && (trx.status==="employee_confirmed" || (trx.status==="kurang_bayar" && rc.isKurang)) && (
                <div style={{marginTop:16,padding:14,borderRadius:"var(--r2)",border:"2px solid", borderColor:rc.isKurang?"#93c5fd":"#6ee7b7", background:rc.isKurang?"#eff6ff":"#f0fdf4"}}>
                  <p style={{fontSize:13,fontWeight:800,marginBottom:4, color:rc.isKurang?"#1e3a8a":"#065f46"}}>
                    {rc.isKurang?"💙 Kurang Bayar — Transfer ke Karyawan":"💚 Pas/Lebih — Konfirmasi Lunas"}
                  </p>
                  ...
                  <button className="btn bg" onClick={settle} disabled={busy||(rc.isKurang&&!norek)||(!settleVoucher)}>
                    <Ic n="check" s={13}/> {rc.isKurang?"Konfirmasi Sudah Transfer":"Konfirmasi Lunas"}
                  </button>
                </div>
              )}
            </>
