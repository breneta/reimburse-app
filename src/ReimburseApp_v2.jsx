...
// Fungsi menghitung selisih hari kerja (mengabaikan Sabtu & Minggu)
const wdiff = (start, end) => {
  ...
  while (d1 < d2) {
    ...
    if (d1.getDay() !== 0 && d1.getDay() !== 6) days++; // 0=Minggu, 6=Sabtu
  }
  return days;
};
...
// ═══════════════════════════════════════════════════════════════
// STYLES (Dipersingkat untuk template, gunakan CSS yang sama)
// ═══════════════════════════════════════════════════════════════
const CSS = `
...
const IP = {
  ...
  bell:"M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0", logout:"M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4M16 17l5-5-5-5M21 12H9",
  menu:"M3 12h18M3 6h18M3 18h18", refresh:"M23 4v6h-6M1 20v-6h6M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15",
...
// ═══════════════════════════════════════════════════════════════
// KOMPONEN (LoginScreen tetap sama)
// ═══════════════════════════════════════════════════════════════
function LoginScreen({ onLogin }) {
  // ... (Gunakan kode LoginScreen aslimu yang tidak diubah)
  const [tab,setTab]=useState("karyawan");const [mode,setMode]=useState("login");const [err,setErr]=useState("");const [show,setShow]=useState(false);const [show2,setShow2]=useState(false);const [username,setUsername]=useState("");const [pass,setPass]=useState("");const [regName,setRegName]=useState("");const [regDept,setRegDept]=useState("");const [regUser,setRegUser]=useState("");const [regPass,setRegPass]=useState("");const [regPass2,setRegPass2]=useState("");const [role,setRole]=useState("approver");const [staffPass,setStaffPass]=useState("");const clr=()=>setErr("");const [busy2,setBusy2]=useState(false);
  const doRegister=async()=>{if(!regName.trim())return setErr("Nama tidak boleh kosong");if(!regDept)return setErr("Pilih departemen dulu");if(!regUser.trim())return setErr("Username tidak boleh kosong");if(regUser.includes(" "))return setErr("Username tidak boleh mengandung spasi");if(regPass.length<4)return setErr("Password minimal 4 karakter");if(regPass!==regPass2)return setErr("Konfirmasi password tidak cocok");const ukey=regUser.toLowerCase().trim();const av=regName.trim().split(" ").map(w=>w[0]).join("").toUpperCase().slice(0,2);const newAcc={username:ukey,name:regName.trim(),dept:regDept,avatar:av,password:regPass};setBusy2(true);if(CONFIG.SCRIPT_URL){const res=await API.registerAcc(newAcc);setBusy2(false);if(!res?.ok)return setErr(res?.error||"Username sudah dipakai, pilih yang lain");}else{const accounts=lsGet2();if(accounts[ukey]){setBusy2(false);return setErr("Username sudah dipakai");}accounts[ukey]=newAcc;lsSave2(accounts);setBusy2(false);}onLogin({name:newAcc.name,dept:newAcc.dept,role:"employee",avatar:av});};
  const doLogin=async()=>{if(!username.trim())return setErr("Masukkan username");if(!pass)return setErr("Masukkan password");const ukey=username.toLowerCase().trim();setBusy2(true);if(CONFIG.SCRIPT_URL){const res=await API.loginAcc(ukey,pass);setBusy2(false);if(!res?.ok)return setErr(res?.error||"Username atau password salah");onLogin({name:res.acc.name,dept:res.acc.dept,role:"employee",avatar:res.acc.avatar});}else{const accounts=lsGet2();const acc=accounts[ukey];setBusy2(false);if(!acc)return setErr("Username tidak ditemukan");if(acc.password!==pass)return setErr("Password salah!");onLogin({name:acc.name,dept:acc.dept,role:"employee",avatar:acc.avatar});}};
  const doStaff=()=>{const correct=role==="approver"?CONFIG.PASS_APPROVER:CONFIG.PASS_FINANCE;if(staffPass!==correct)return setErr("Password salah!");const info=role==="approver"?{name:"Approver",dept:"Management",avatar:"AP"}:{name:"Finance",dept:"Finance",avatar:"FN"};onLogin({...info,role});};
  const PwInput=({value,onChange,placeholder,showState,toggleShow,onEnter})=>(<div style={{position:"relative"}}><input type={showState?"text":"password"} value={value} onChange={e=>{onChange(e.target.value);clr();}} placeholder={placeholder} onKeyDown={e=>e.key==="Enter"&&onEnter&&onEnter()} style={{paddingRight:42}}/><button onClick={toggleShow} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",fontSize:16,color:"var(--i3)"}}>{showState?"🙈":"👁️"}</button></div>);
  return(<div className="lw"><div className="lr1"/><div className="lr2"/><div className="lc"><div style={{textAlign:"center",marginBottom:24}}><div className="l-ico">💼</div><h1 style={{fontFamily:"'Playfair Display',serif",fontSize:22,fontStyle:"italic",color:"var(--ink)"}}>ReimburseApp</h1><p style={{fontSize:12,color:"var(--i3)",marginTop:3}}>Sistem Reimburse & Cash Advance</p></div><div className="l-tabs">{[["karyawan","👤  Karyawan"],["staff","🔐  Admin / Finance"]].map(([v,l])=>(<button key={v} className={`l-tab${tab===v?" on":""}`} onClick={()=>{setTab(v);setErr("");setMode("login");}}>{l}</button>))}</div>{err&&<div className="l-err"><Ic n="x" s={13} c="#dc2626"/>{err}</div>}{tab==="karyawan"&&mode==="login"&&(<><div className="l-fld"><label>Username</label><input value={username} onChange={e=>{setUsername(e.target.value);clr();}} placeholder="Username yang sudah didaftarkan" autoFocus onKeyDown={e=>e.key==="Enter"&&doLogin()}/></div><div className="l-fld"><label>Password</label><PwInput value={pass} onChange={setPass} placeholder="Password kamu" showState={show} toggleShow={()=>setShow(s=>!s)} onEnter={doLogin}/></div><button className="l-btn" onClick={doLogin} disabled={busy2}>{busy2?<span className="sp2"/>:"Masuk →"}</button><div style={{textAlign:"center",marginTop:14}}><span style={{fontSize:12.5,color:"var(--i3)"}}>Belum punya akun? </span><button onClick={()=>{setMode("register");setErr("");}} style={{background:"none",border:"none",cursor:"pointer",fontSize:12.5,fontWeight:700,color:"var(--tl)",fontFamily:"inherit"}}>Daftar sekarang</button></div></>)}{tab==="karyawan"&&mode==="register"&&(<><div style={{background:"var(--tlb)",border:"1px solid var(--tlbd)",borderRadius:"var(--r3)",padding:"9px 12px",marginBottom:14,fontSize:12,color:"#134e4a"}}>✨ Daftar sekali, langsung bisa login kapan saja dari HP atau laptop.</div><div className="l-fld"><label>Nama Lengkap <span style={{color:"var(--rd)"}}>*</span></label><input value={regName} onChange={e=>{setRegName(e.target.value);clr();}} placeholder="Nama lengkap kamu" autoFocus/></div><div className="l-fld"><label>Departemen <span style={{color:"var(--rd)"}}>*</span></label><select value={regDept} onChange={e=>{setRegDept(e.target.value);clr();}}><option value="">-- Pilih Departemen --</option>{DEPTS.map(d=><option key={d}>{d}</option>)}</select></div><div className="l-fld"><label>Username <span style={{color:"var(--rd)"}}>*</span></label><input value={regUser} onChange={e=>{setRegUser(e.target.value);clr();}} placeholder="Contoh: budi.santoso"/><p style={{fontSize:11,color:"var(--i3)",marginTop:3}}>Huruf kecil, tanpa spasi. Dipakai untuk login berikutnya.</p></div><div className="l-fld"><label>Password <span style={{color:"var(--rd)"}}>*</span></label><PwInput value={regPass} onChange={setRegPass} placeholder="Min. 4 karakter" showState={show} toggleShow={()=>setShow(s=>!s)}/></div><div className="l-fld"><label>Konfirmasi Password <span style={{color:"var(--rd)"}}>*</span></label><PwInput value={regPass2} onChange={setRegPass2} placeholder="Ulangi password" showState={show2} toggleShow={()=>setShow2(s=>!s)} onEnter={doRegister}/></div><button className="l-btn" onClick={doRegister} disabled={busy2}>{busy2?<span className="sp2"/>:"Daftar & Masuk →"}</button><div style={{textAlign:"center",marginTop:14}}><span style={{fontSize:12.5,color:"var(--i3)"}}>Sudah punya akun? </span><button onClick={()=>{setMode("login");setErr("");}} style={{background:"none",border:"none",cursor:"pointer",fontSize:12.5,fontWeight:700,color:"var(--tl)",fontFamily:"inherit"}}>Login di sini</button></div></>)}{tab==="staff"&&(<><div className="l-fld"><label>Login sebagai</label><select value={role} onChange={e=>{setRole(e.target.value);clr();}}><option value="approver">✅  Approver / Admin</option><option value="finance">💼  Finance</option></select></div><div className="l-fld"><label>Password <span style={{color:"var(--rd)"}}>*</span></label><PwInput value={staffPass} onChange={setStaffPass} placeholder="Masukkan password" showState={show} toggleShow={()=>setShow(s=>!s)} onEnter={doStaff}/></div><button className="l-btn" onClick={doStaff}>Masuk →</button><div className="l-note">🔑 Password default: <strong>approver123</strong> / <strong>finance123</strong><br/>Ganti di menu <strong>Pengaturan</strong> (login Finance).</div></>)}</div></div>);
}
...
function Dashboard({ data, user, nav }) {
  ...
  // Menggunakan perhitungan wdiff dinamis untuk badge keterlambatan di dashboard
  const overdue = mine.filter(d=>d.type==="cash_advance" && !d.settled && !["rejected"].includes(d.status) && wdiff(d.dateEnd, today()) > 5);
...
function SubmitPage({ user, onSubmit }) {
  ...
  // Menghitung hari keterlambatan OER / CA saat di form 
  const lateDays = f.dateEnd ? wdiff(f.dateEnd, today()) : 0;
    ...
    <div><div className="card">
      ...
      <div className="cb">
        <div className="fs mb4">
          ...
          <div className="al aw mt3"><Ic n="clock" s={13} c="#d97706"/><span>CA settlement & OER wajib diajukan <strong>maks. 5 hari kerja</strong> setelah selesai.</span></div>
        </div>
        <div className="fs mb4">
          ...
          {lateDays > 5 && (
            <div className="al ae mt3">
              <Ic n="alert" s={13} c="#dc2626"/>
              <span>⚠️ <strong>Perhatian:</strong> Pengajuan (CA/OER) ini sudah lewat batas 5 hari kerja setelah trip!</span>
            </div>
          )}
        </div>
        <div className="fs mb4">
...
// ═══════════════════════════════════════════════════════════════
// APPROVAL PAGE (Dengan penambahan fitur pencarian Karyawan)
// ═══════════════════════════════════════════════════════════════
function ApprovalPage({ data, onAction }) {
  const [search, setSearch] = useState("");
  // Menampilkan hanya pending, DAN cocok dengan pencarian nama submitter
  const queue = data.filter(d => 
    d.status === "pending" && 
    (!search || d.submitter.toLowerCase().includes(search.toLowerCase()))
  );
  
  return (
    <div>
      {data.filter(d=>d.status==="pending").length>0 && <div className="al aw mb4"><Ic n="clock" s={14} c="#d97706"/><span><strong>{data.filter(d=>d.status==="pending").length} pengajuan total</strong> sedang menunggu persetujuan Anda.</span></div>}
      
      {/* Search Bar Tambahan */}
      <div className="card mb4" style={{padding:"11px 16px"}}>
        <div className="flt">
          <input placeholder="🔍 Cari nama Karyawan yang ingin di-approve..." value={search} onChange={e=>setSearch(e.target.value)} style={{flex:2}}/>
          {search && <button className="btn bo sm" onClick={()=>setSearch("")}><Ic n="x" s={12}/>Reset</button>}
        </div>
      </div>

      <div className="card">
        <div className="ch"><h3>Antrian Approval <span style={{fontSize:12,color:"var(--i4)",fontWeight:400}}>({queue.length})</span></h3></div>
        <div className="tw"><table>
        ...
        </table>{queue.length===0&&<div className="empty"><Ic n="check" s={36}/><p style={{marginTop:10}}>Tidak ada antrian yang sesuai 🎉</p></div>}
        </div>
      </div>
    </div>
  );
...
// ═══════════════════════════════════════════════════════════════
// Sisa Komponen App (Gunakan kembali EditForm, Modal, dsb. secara persis)
// ═══════════════════════════════════════════════════════════════
// (Kode MonitorPage, EditForm, DetailModal, SettingsPage disalin persis seperti file asli kamu)
...
function MonitorPage({ data, onSel }) {
  ...
  // Update overdue check with wdiff
  const overdue = data.filter(d=>d.type==="cash_advance" && !d.settled && !["rejected"].includes(d.status) && wdiff(d.dateEnd, today()) > 5);
          ...
          <div style={{maxHeight:340,overflowY:"auto"}}>
            {caOut.map(d=>(
              <div key={d.id} onClick={()=>onSel(d.id)} style={{padding:"11px 16px",borderBottom:"1px solid var(--ln)",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div><span className="mono">{d.id}</span><div className="bold" style={{fontSize:13}}>{d.submitter}</div><div style={{fontSize:11,color:"var(--i3)"}}>Selesai: {fd(d.dateEnd)}</div></div>
...
// ... COPY SETTINGSPAGE, EDITFORM, DAN DETAILMODAL ASLI DI SINI ...
function SettingsPage({ onSave }) {
  ...
  const save = () => { CONFIG.SCRIPT_URL=url.trim(); CONFIG.PASS_APPROVER=pa; CONFIG.PASS_FINANCE=pf; setSaved(true); setTimeout(()=>setSaved(false),2500); if(onSave)onSave(); };
  return (<div><div className="card mb4"><div className="ch"><h3>Koneksi Google Sheets</h3></div><div className="cb"><div className="al ab mb4"><Ic n="settings" s={14} c="#2563eb"/><span>Isi URL Apps Script agar data karyawan langsung tersimpan ke Google Sheets.</span></div><div className="fg mb4"><label className="fl">Apps Script Web App URL</label><input value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://script.google.com/macros/s/xxxxx/exec"/></div><div style={{padding:"10px 13px",background:"var(--ln2)",borderRadius:"var(--r2)",border:"1px solid var(--ln)"}}><p style={{fontSize:11,fontWeight:700,color:"var(--i3)",marginBottom:3}}>STATUS</p>{CONFIG.SCRIPT_URL?<span style={{color:"var(--gn)",fontWeight:700,fontSize:13}}>✓ Terhubung ke Google Sheets</span>:<span style={{color:"var(--am)",fontWeight:700,fontSize:13}}>⚠️ Belum terhubung</span>}</div></div></div><div className="card"><div className="ch"><h3>Password Login</h3></div><div className="cb"><div className="fg fg2 mb4"><div><label className="fl">Password Approver</label><input value={pa} onChange={e=>setPa(e.target.value)} placeholder="approver123"/></div><div><label className="fl">Password Finance</label><input value={pf} onChange={e=>setPf(e.target.value)} placeholder="finance123"/></div></div><div style={{display:"flex",justifyContent:"flex-end",gap:9,alignItems:"center"}}>{saved&&<span style={{color:"var(--gn)",fontWeight:700,fontSize:13}}>✓ Disimpan!</span>}<button className="btn bp" onClick={save}><Ic n="check" s={13}/>Simpan Perubahan</button></div></div></div></div>);
}

function EditForm({ trx, user, onSave, onCancel }) {
  const [f,setF] = useState({type:trx.type,purpose:trx.purpose,destination:trx.destination,dateStart:trx.dateStart,dateEnd:trx.dateEnd,approverName:trx.approverName,notes:trx.notes||"",items:trx.categories.map(c=>({cat:c.cat, amt:String(c.amt)}))});
  const [busy,setBusy] = useState(false);
  const set=(k,v)=>setF(p=>({...p,[k]:v}));
  const si=(i,k,v)=>setF(p=>{const it=[...p.items];it[i]={...it[i],[k]:v};return{...p,items:it};});
  const total = f.items.reduce((a,it)=>a+(parseFloat(it.amt)||0),0);
  const save=async()=>{if(!f.purpose||!f.dateStart||!f.dateEnd||!f.approverName||total===0){alert("Lengkapi field wajib.");return;}setBusy(true);const updated={...trx,type:f.type,purpose:f.purpose,destination:f.destination,dateStart:f.dateStart,dateEnd:f.dateEnd,approverName:f.approverName,notes:f.notes,amount:total,categories:f.items.map(it=>({cat:it.cat, amt:parseFloat(it.amt)||0}))};if(CONFIG.SCRIPT_URL)await API.editData(trx.id, updated);else await new Promise(r=>setTimeout(r,400));setBusy(false);onSave(updated);};
  return (<div style={{padding:"4px 0"}}><div className="fs mb3"><div className="fst">Jenis</div><div style={{display:"flex",gap:9}}>{[["reimburse","💰 Reimburse"],["cash_advance","🏦 CA"]].map(([v,l])=>(<label key={v} style={{flex:1,display:"flex",alignItems:"center",gap:8,padding:"9px 12px",borderRadius:"var(--r2)",border:`2px solid ${f.type===v?"var(--tl)":"var(--ln)"}`,background:f.type===v?"var(--tlb)":"var(--w)",cursor:"pointer",margin:0}}><input type="radio" checked={f.type===v} onChange={()=>set("type",v)} style={{width:"auto",accentColor:"var(--tl)"}}/><span style={{fontSize:13,fontWeight:700}}>{l}</span></label>))}</div></div><div className="fs mb3"><div className="fst">Detail</div><div className="fg mb3"><label className="fl">Keperluan *</label><textarea value={f.purpose} onChange={e=>set("purpose",e.target.value)} rows={2}/></div><div className="fg fg3"><div><label className="fl">Tujuan</label><input value={f.destination} onChange={e=>set("destination",e.target.value)}/></div><div><label className="fl">Mulai</label><input type="date" value={f.dateStart} onChange={e=>set("dateStart",e.target.value)}/></div><div><label className="fl">Selesai</label><input type="date" value={f.dateEnd} onChange={e=>set("dateEnd",e.target.value)}/></div></div></div><div className="fs mb3"><div className="fst">Biaya</div>{f.items.map((it,i)=>(<div key={i} style={{display:"flex",gap:9,alignItems:"flex-end",marginBottom:8}}><div style={{flex:2}}><select value={it.cat} onChange={e=>si(i,"cat",e.target.value)}>{CATS.map(c=><option key={c}>{c}</option>)}</select></div><div style={{flex:1.5}}><input type="number" value={it.amt} onChange={e=>si(i,"amt",e.target.value)}/></div><button className="btn bo xs" onClick={()=>setF(p=>({...p,items:p.items.filter((_,j)=>j!==i)}))}><Ic n="trash" s={12}/></button></div>))}<button className="btn bo sm" onClick={()=>setF(p=>({...p,items:[...p.items,{cat:"Perjalanan Dinas",amt:""}]}))}>Tambah</button></div><div style={{display:"flex",justifyContent:"flex-end",gap:9}}><button className="btn bo" onClick={onCancel}>Batal</button><button className="btn bp" onClick={save} disabled={busy}>{busy?"Simpan...":"Simpan"}</button></div></div>);
}

function DetailModal({ trx, user, onClose, onAction, onEdit }) {
  const [note,setNote]=useState("");const [busy,setBusy]=useState(false);const [editing,setEditing]=useState(false);
  const act=async(action,n)=>{setBusy(true);const sm={approve:"approved",reject:"rejected",process:"processing",pay:"paid"};if(CONFIG.SCRIPT_URL)await API.update(trx.id, sm[action], n);else await new Promise(r=>setTimeout(r,400));setBusy(false);onAction(trx.id, action, n);};
  const settle=async()=>{setBusy(true);if(CONFIG.SCRIPT_URL)await API.settle(trx.id,note);else await new Promise(r=>setTimeout(r,400));setBusy(false);onAction(trx.id,"settle",note);};
  const tl=[{ok:true,icon:"send",title:"Diajukan",sub:trx.submitter,col:"var(--tl)"},{ok:!["pending"].includes(trx.status),icon:"user",title:"Approval",sub:trx.status==="pending"?"Menunggu":trx.approverName,col:trx.status==="pending"?"var(--am)":"var(--gn)"},{ok:["processing","paid"].includes(trx.status),icon:"money",title:"Finance",sub:trx.status==="processing"?"Proses":"Selesai",col:trx.status==="paid"?"var(--gn)":"var(--i4)"},{ok:trx.status==="paid",icon:"check",title:"Pembayaran",sub:trx.status==="paid"?"Selesai":"Menunggu",col:trx.status==="paid"?"var(--gn)":"var(--i4)"}];
  return(<div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}><div className="mo"><div className="mh"><div style={{flex:1}}><span className="mono">{trx.id}</span><h2 style={{fontSize:15,fontWeight:800,marginTop:3}}>{trx.purpose}</h2></div><div style={{display:"flex",gap:6,alignItems:"center"}}>{(user.role==="finance"||(user.role==="employee"&&trx.submitter===user.name&&trx.status!=="paid"))&&!editing&&<button className="btn bo sm" onClick={()=>setEditing(true)}>✏️ Edit</button>}<button className="btn bo sm" onClick={onClose}><Ic n="x" s={13}/></button></div></div><div className="mb2">{editing?<EditForm trx={trx} user={user} onSave={u=>{setEditing(false);onEdit(u);}} onCancel={()=>setEditing(false)}/>:(<><div className="g2 mb4"><div><p className="bold">{trx.submitter}</p><p style={{fontSize:12,color:"var(--i3)"}}>{trx.dept}</p></div><div><p className="bold">{trx.destination}</p><p style={{fontSize:12,color:"var(--i3)"}}>{fd(trx.dateStart)} – {fd(trx.dateEnd)}</p></div></div><div style={{border:"1px solid var(--ln)",borderRadius:"var(--r2)",overflow:"hidden",marginBottom:16}}>{trx.categories.map((c,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"9px 14px",fontSize:13}}><span>{c.cat}</span><span className="bold">{rp(c.amt)}</span></div>))}<div style={{display:"flex",justifyContent:"space-between",padding:"11px 14px",background:"var(--tlb)",borderTop:"2px solid var(--tl)"}}><span style={{fontWeight:800,color:"var(--tl)"}}>TOTAL</span><span style={{fontWeight:800,fontSize:16,color:"var(--tl)"}}>{rp(trx.amount)}</span></div></div><div>{tl.map((t,i)=>(<div key={i} className="tlr"><div className="tldc"><div className="tld" style={{background:t.ok?t.col:"var(--ln)"}}><Ic n={t.icon} s={12} c={t.ok?"#fff":"var(--i4)"}/></div>{i<tl.length-1&&<div className="tlln"/>}</div><div className="tlb"><div className="tlt">{t.title}</div><div className="tls">{t.sub}</div></div></div>))}</div>{user.role==="approver"&&trx.status==="pending"&&(<div style={{marginTop:16,padding:14,background:"var(--ln2)"}}><div style={{display:"flex",gap:8}}><button className="btn bg" onClick={()=>act("approve","Disetujui")} disabled={busy}>Setujui</button><button className="btn br2" onClick={()=>act("reject","Ditolak")} disabled={busy}>Tolak</button></div></div>)}{user.role==="finance"&&trx.status==="approved"&&(<div style={{marginTop:16}}><button className="btn bp" onClick={()=>act("process","Proses")}>Mulai Proses</button></div>)}{user.role==="finance"&&trx.status==="processing"&&(<div style={{marginTop:16}}><button className="btn bg" onClick={()=>act("pay","Lunas")}>Tandai Dibayar</button></div>)}{user.role==="finance"&&trx.status==="paid"&&trx.type==="cash_advance"&&!trx.settled&&(<div style={{marginTop:16}}><button className="btn bg" onClick={settle}>Konfirmasi Settlement</button></div>)}</>)}</div></div></div>);
}
...
export default function App() {
  ...
  // Dynamic over-due hitungan Wdiff
  const oCt = data.filter(d=>d.type==="cash_advance" && !d.settled && !["rejected"].includes(d.status) && wdiff(d.dateEnd, today()) > 5).length;
    ...
    <>
      ...
      <div className="app">
        ...
        <div className={`sb${sideOpen?" open":""}`}>
          ...
          <div className="sb-u" onClick={handleLogout} title="Klik untuk logout"><div className="av">{user.avatar}</div><div style={{flex:1,minWidth:0}}><div className="sb-un">{user.name}</div><div className="sb-ur">{user.dept}</div><div className="sb-lo"><Ic n="logout" s={10} c="rgba(255,255,255,.25)"/>Tap untuk logout</div></div></div>
          <nav className="sb-nav">
            <div className="nv-s">Menu</div>
            {(NAV[user.role]||[]).map(item=>(<div key={item.id} className={`nv${page===item.id?" on":""}`} onClick={()=>nav(item.id)}><Ic n={item.ic} s={14}/><span style={{flex:1}}>{item.lb}</span>{item.bd>0 && <span className="nb">{item.bd}</span>}</div>))}
          </nav>
        </div>
        <div className="main">
          <div className="bar">
            ...
            <div className="br">
              <span className={`cs ${CONFIG.SCRIPT_URL?"cs-ok":"cs-no"}`}><span style={{width:6,height:6,borderRadius:"50%",background:CONFIG.SCRIPT_URL?"var(--gn)":"var(--am)",display:"inline-block"}}/>{CONFIG.SCRIPT_URL?"Sheets ✓":"Lokal"}</span>
              {user.role==="employee"&&page!=ass="submit"&&<button className="btn bp sm" onClick={()=>nav("submit")}><Ic n="plus" s={13}/>Ajukan</button>}
            </div>
          </div>
          <div className="page">
            {loading ? (
                    ...
                    <div className="card">
                      ...
                      <div className="tw"><table>
                        ...
                        <tbody>{data.filter(d=>d.type==="cash_advance"&&!d.settled&&!["rejected"].includes(d.status)).map(d=>{
                          // Implementasi hari kerja di Overdue Page 
                          const late = Math.max(0, wdiff(d.dateEnd, today()) - 5);
                          return (
